...
import { InMemoryEventBus, type ImpactEventPublisher, type ImpactEventSubscriber } from "../../lib/impactos/event-bus";

test("event bus publishes typed module events without module coupling", async () => {
  const bus = new InMemoryEventBus();
  const publisher: ImpactEventPublisher = bus;
  const subscriber: ImpactEventSubscriber = bus;
  const received: string[] = [];

  const unsubscribe = subscriber.subscribe("document.created", async (event) => {
