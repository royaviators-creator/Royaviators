    ...
    >
      <div className="model-tier model-grid">
        {operatingInputs.map((item) => {
          ...
          return (
            <div className="model-node" tabIndex={0} key={item.title}>
      ...
      <div className="model-core" tabIndex={0}>
      ...
      <div className="model-tier model-grid">
        {operatingOutputs.map((output) => (
          <div className="model-node" tabIndex={0} key={output}>
            <span>{output}</span>
          </div>
        ))}
      </div>
      <span className="model-line" aria-hidden="true" />
      <div className="model-outcome" tabIndex={0}>Better Decisions</div>
      <span className="model-line short" aria-hidden="true" />
      <div className="model-impact" tabIndex={0}>Measurable Impact</div>
    </div>
  );
}
...
export function PageCTA({
  ...
  text = "Book a strategy session to explore where clearer systems can improve decisions, accountability, and long-term progress.",
}: {
  eyebrow?: string;
  title?: string;
  text?: string;
